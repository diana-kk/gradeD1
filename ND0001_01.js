...
function handlerNext() {
  location.replace('ND0001_02.html');
}

// 상단
var topArea = Paper.g();

topArea.text(180, 85, '1 / 8').attr({
  'font-size': 20,
  'fill': 'gray',
  'text-anchor': 'middle'
});

topArea.text(143, 160, 'あ').click(playAud01).attr({
  'font-size': 30,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});

var t1 = topArea.text(180, 250, ['会う', '만나다']).click(playAud01).attr({
  'font-size': 90,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
bottomArea.text(180, 462, '자주 만나?').attr({
  'font-size': 18,
  'text-anchor': 'middle'
});
...
function handler() {
  ...
  var t3 = bottomArea.text(180, 540, ['よく', ' 会う', '？']).click(naru).attr({
    'font-size': 30,
    'fill': 'gray',
    'text-anchor': 'middle'
  });
  ...
  function naru() {
    var audio3 = new Audio('aud/sen01.wav');
    audio3.play();
  }
