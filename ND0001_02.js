...
function handlerNext() {
  location.replace('ND0001_03.html');
}

// 상단
var topArea = Paper.g();

topArea.text(180, 85, '2 / 8').attr({
  'font-size': 20,
  'fill': 'gray',
  'text-anchor': 'middle'
});

topArea.text(143, 160, 'ま').click(playAud01).attr({
  'font-size': 30,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});

var t1 = topArea.text(180, 250, ['待つ', '기다리다']).click(playAud01).attr({
  'font-size': 90,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
bottomArea.text(180, 462, '매일 기다려.').attr({
  'font-size': 18,
  'text-anchor': 'middle'
});
...
function handler() {
  ...
  var t3 = bottomArea.text(185, 540, ['毎日', ' 待つ', '。']).click(naru).attr({
    'font-size': 30,
    'fill': 'gray',
    'text-anchor': 'middle'
  });
  ...
  function naru() {
    var audio3 = new Audio('aud/sen02.wav');
    audio3.play();
  }
