...
function handlerNext() {
  location.replace('ND0001_04.html');
}

// 상단
var topArea = Paper.g();

topArea.text(180, 85, '3 / 8').attr({
  'font-size': 20,
  'fill': 'gray',
  'text-anchor': 'middle'
});

topArea.text(140, 160, 'つく').click(playAud01).attr({
  'font-size': 30,
  'text-anchor': 'middle',
  'letter-spacing': 5,
  'cursor': 'pointer'
...
var t1 = topArea.text(180, 250, ['作る', '만들다']).click(playAud01).attr({
  'font-size': 90,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function playAud01() {
  var audio1 = new Audio('aud/voca03.wav');
  audio1.play();
}
...
bottomArea.text(180, 462, '매일 만들어?').attr({
  'font-size': 18,
  'text-anchor': 'middle'
});
...
function handler() {
  ...
  var t3 = bottomArea.text(185, 540, ['毎日', ' 作る', '？']).click(naru).attr({
    'font-size': 30,
    'fill': 'gray',
    'text-anchor': 'middle'
  });
  ...
  function naru() {
    var audio3 = new Audio('aud/sen03.wav');
    audio3.play();
  }

  var audio2 = new Audio('aud/sen03.wav');
  audio2.play();
}
